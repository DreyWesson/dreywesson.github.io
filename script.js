...
getRepo.addEventListener("submit", (e) => {
  ...
  async function gitHub(user) {
    try {
      ...
      const response = fetch(
        `https://api.github.com/users/${user}?per_page=100`
      );
      ...
      console.log(
        `${user}'s most forked repo >=50 ${emojiProcessor(unifiedValue[3])}`
      );
      data.map((element, index) => {
        if (element.forks >= 50)
          console.log(
            `${index}. ${element.full_name} && URL: ${element.html_url}`
          );
      });
    } catch (error) {
  ...
  document.querySelector(
    "#repo-result"
  ).innerHTML = `Geeky stuff. Check <span>${value}'s</span> most forked repo in the console!!!`;
  winSound.play();
});
...
function rpsGame(yourChoice) {
  ...
  if (botChoice === yourChoice.id) {
    message = `Bot chose ${botChoice}. You chose ${
      yourChoice.id
    }. You drew ${emojiProcessor(unifiedValue[0])}`;
    messageColor = "#657786";
  } else if (
    (botChoice == "rock" && yourChoice.id == "scissors") ||
    (botChoice == "paper" && yourChoice.id == "rock") ||
    (botChoice == "scissors" && yourChoice.id == "paper")
  ) {
    message = `Bot chose ${botChoice}. You chose ${
      yourChoice.id
    }. Bot wins ${emojiProcessor(unifiedValue[2])}!`;
    messageColor = "#dc3545";
    lossSound.play();
  } else {
    message = `Bot chose ${botChoice}. You chose ${
      yourChoice.id
    }. You win ${emojiProcessor(unifiedValue[1])}!`;
    messageColor = "green";
    winSound.play();
  }
...
function buttonColorChange(tags) {
  if (tags.value == "red") buttonsRed();
  else if (tags.value == "green") buttonsGreen();
  else if (tags.value == "reset") buttonsColorReset();
  else if (tags.value == "random") randomColors();
}
...
document
  ...
  .addEventListener("click", () => botLogic());
