...
getRepo.addEventListener('submit', (e)=> {
  ...
  async function gitHub(user) {
    try {const res = fetch(`https://api.github.com/users/${user}/repos`);
      ...
      console.log(`User's most forked repo`);
      data.map((element, index) => {
        if (element.forks >= 50) 
  ...
  gitHub(value);
  document.querySelector('#repo-result').innerHTML = `Geeky stuff. Git <span>${value}'s</span> repo(s) in the console!!!`
  winSound.play()
})
...
//add book
  const getBookForm = document.forms['add-book'];
  getBookForm.addEventListener('submit', (e)=> {
    e.preventDefault()
    const value = getBookForm.querySelector('input[type="text"]').value;
